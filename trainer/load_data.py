...
def load_train_data(noise_list=[30], doppler=[75], tr_percent=0.8, phase='train', data_mode= ['CDL-A'], RX_Ants_list=[4]):
    ...
    de tenseurs prêts pour l'entraînement (concaténés, permutés, splittés, etc.).
    """
    t_start = time.time()

    # Pour compatibilité : on tolère la signature avec un seul élément ou une liste
    if isinstance(noise_list, int): noise_list = [noise_list]
    if isinstance(doppler, int): doppler = [doppler]
    if isinstance(data_mode, str): data_mode = [data_mode]
    if isinstance(RX_Ants_list, int): RX_Ants_list = [RX_Ants_list]

    # Accumulateurs de tous les tenseurs à concaténer
    all_Received_Y, all_Hls, all_Ideal_H, all_Ideal_X, all_Trans_X = [], [], [], [], []
    all_sigma = []
    total_samples = 0
        ...
        for dop in doppler:
            for rx in RX_Ants_list:
                for snr in noise_list:
                    ...
                    # Chargement + cast/permute
                    Received_Y = torch.from_numpy(mat['Received_Y']).permute(0, 1, 4, 2, 3).float()
                    Hls        = torch.from_numpy(mat['Hls']).permute(0, 1, 4, 2, 3).float()
                    Ideal_H    = torch.from_numpy(mat['Ideal_H']).permute(0, 1, 4, 2, 3).float()
                    Ideal_X    = torch.from_numpy(mat['Ideal_X']).permute(0, 3, 1, 2).float()
                    Trans_X    = torch.from_numpy(mat['Transmit_X']).permute(0, 3, 1, 2).float()

                    all_Received_Y.append(Received_Y)
                    all_Hls.append(Hls)
                    all_Ideal_H.append(Ideal_H)
                    all_Ideal_X.append(Ideal_X)
                    all_Trans_X.append(Trans_X)
                    
                    # sigma pour chaque batch (un seul par config, donc rempli sur la taille locale)
                    n_samples = Received_Y.shape[0]
                    sigma = torch.full((n_samples, 1, 1), np.power(10, -snr/10), dtype=torch.float32)
                    all_sigma.append(sigma)
                    total_samples += n_samples

    # Concaténation globale sur toutes les configs
    Received_Y = torch.cat(all_Received_Y, dim=0)
    Hls        = torch.cat(all_Hls, dim=0)
    Ideal_H    = torch.cat(all_Ideal_H, dim=0)
    Ideal_X    = torch.cat(all_Ideal_X, dim=0)
    Trans_X    = torch.cat(all_Trans_X, dim=0)
    sigma      = torch.cat(all_sigma, dim=0)

    # Split train selon tr_percent
    idx_tr = int(Received_Y.shape[0] * tr_percent)
    Yms_tr  = Received_Y[:idx_tr, ...]
    Hls_tr  = Hls[:idx_tr, ...]
    Hid_tr  = Ideal_H[:idx_tr, ...]
    ...
    # Xid_tr = Xid_tr.permute(0,2,1,3).unsqueeze(-1)
    X0_tr = X0_tr.permute(0,2,1,3).unsqueeze(-1)
    
    print(f'Load data time cost={end_time:.2f}s, train dataset len={train_size}')
    print('Shapes :')
    print(' Yms_tr ', Yms_tr.shape)
    print(' Hls_tr ', Hls_tr.shape)
    print(' Hid_tr ', Hid_tr.shape)
    print(' Xid_tr ', Xid_tr.shape)
    print(' X0_tr  ', X0_tr.shape)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.x[index], self.snr[index],  self.dop[index], self.label[index]
...
def load_data(dop=75,snr=[0],data_per=1.0,phase='train',dataset_name=['CDL-A'],RX_Ants_list=[4]):
    ...
    - Pour valid/test, passer les configs précises à tester.
    """
    # Pour l'entraînement : SNR utilisés uniquement pour le TRAIN snr=[-5, 0, 5]
    if phase=='train':
        Yms_tr, Hls_tr, Hid_tr, Xid_tr, X0_tr, sigma, _ = load_train_data(
        noise_list=snr,
        doppler=[dop],
        tr_percent=data_per,
        phase=phase,
        data_mode=dataset_name,
        RX_Ants_list=RX_Ants_list
    )
        
    # Pour la validation SNR de validation spécifique 0
    elif phase=='valid':
        Yms_tr, Hls_tr, Hid_tr, Xid_tr, X0_tr, sigma, _ = load_train_data(
        noise_list=snr,
        doppler=[dop],
    ...
    # Pour le test SNR de test spécifique, jamais utilisé avant 10  
    elif phase=='test':
        Yms_tr, Hls_tr, Hid_tr, Xid_tr, X0_tr, sigma, _ = load_train_data(
        noise_list=snr,
        doppler=[dop],
    ...
    # Maintenant concat sur la dernière dim (channels)
    input_x = torch.cat([Received_Y, Transmit_X, Hls], dim=-1) 
    print('input_x.shape=', input_x.shape)
     
    input_x = input_x.permute(0, 2, 4, 1, 3).reshape(input_x.shape[0],2*9, 12, 24)
    #input_x = input_x.permute(0, 1, 3, 2, 4).reshape(input_x.shape[0],12, 24, 2*9)
    print('input_x.shape=', input_x.shape)



    #input_x = torch.cat([Received_Y, Transmit_X, Hls], axis=1)
...
def data_with_phase_shift_new(doppler=75, noise_list=10, f=100, tau=72, tr_percent=0.8, phase='test',data_mode='CDL-A',RX_Ants_list=[4]):
    ...
    # Accumulateurs
    all_Received_Y, all_Hls, all_Ideal_X, all_Trans_X, all_sigma = [], [], [], [], []
        ...
        for dop in doppler:
            for rx in RX_Ants_list:
                for snr in noise_list:
                    ...
                    n_samples = Received_Y.shape[0]
                    # SIGMA bien généré comme dans training, pour CHAQUE sample
                    sigma = torch.full((n_samples, 1, 1), np.power(10, -snr/10), dtype=torch.float32)
                    all_sigma.append(sigma)

    # Concatène toutes les configs
    Received_Y = torch.cat(all_Received_Y, dim=0)
    Hls        = torch.cat(all_Hls, dim=0)
    Ideal_X    = torch.cat(all_Ideal_X, dim=0)
    Trans_X    = torch.cat(all_Trans_X, dim=0)
    ...
    # Split (utile si tu veux, sinon tout prendre)
    idx = int(Received_Y.shape[0] * tr_percent)
    Yms_tr = Received_Y[:idx, ...]
    Hls_tr = Hls[:idx, ...]
    Xid_tr = Ideal_X[:idx, ...]
    X0_tr  = Trans_X[:idx, ...]
    sigma  = sigma[:idx, ...]   # Toujours aligné au split

    end_time = time.time() - t_start
    print(f'Load data time cost={end_time:.2f}s, test dataset len={Yms_tr.shape[0]}')
    print('Shapes:')
    print(' Yms_tr ', Yms_tr.shape)
    print(' Hls_tr ', Hls_tr.shape)
    print(' Xid_tr ', Xid_tr.shape)
...
def load_phase_shift_data(dop, snr=[0], f=100, tau=72, data_per=1.0, phase='train', dataset_name='CDL-A',RX_Ants_list=[4]):
    if phase=='test':
        # Yms_ts,  Hls_ts,  Hid_ts, Xid_ts, X0_ts, sigma_ts, _ = load_train_data(noise_list=snr[0], doppler=dop, tr_percent=data_per, phase=phase,data_mode=dataset_name)
        ...
        Yms_ts,  Hls_ts, Xid_ts, X0_ts, sigma_ts = data_with_phase_shift_new(doppler=dop, noise_list=snr[0], f=f, tau=tau, tr_percent=data_per, phase=phase,data_mode=dataset_name, RX_Ants_list=RX_Ants_list)
        Received_Y=Yms_ts
        Transmit_X=X0_ts
        Hls=Hls_ts
        Ideal_X=Xid_ts
        ...
        print('dop_list.shape=',dop_list.shape)
    # input_x = torch.cat([Received_Y, Transmit_X, Hls], axis=1) 
    # channel = (8+2) + 8 = 10 +8 = 18 (first 10 mainly used)
...
'''def sim_H(doppler=75, snr=10, rx=4, mode='CDL-A'):
    """
    Charge Ideal_H depuis un .mat avec plusieurs variables,
    ex : CDL-A10dB_75Hz_R_4.mat
    """

    mat_path = f'C:/Users/nzide/Desktop/DeepRX/{mode}{snr}dB_{doppler}Hz_R_{rx}.mat'
    mat = loadmat(mat_path)
    Ideal_H = mat['Ideal_H']  # shape: (N, 12, 24, 4, 2)
    Ideal_H = torch.from_numpy(Ideal_H).float()
    Ideal_H = Ideal_H.permute(-1, -2, -3, -4, -5)
    print("Ideal_H.shape =", Ideal_H.shape)
    return Ideal_H
'''
