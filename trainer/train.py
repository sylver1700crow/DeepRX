...
class Trainer():
    ...
    def _set_optim(self):
        ...
        # scheduler = CosineAnnealingLR(optimizer,T_max=self.args.epoch)
    ...
    def tr(self):
        ...
        start = 0
        # Resume Training 
        print('Personal Modification=====', self.args.resume)
        ...
        vl_input, vl_snr, vl_dop, vl_label, vl_Hid = load_data(dop,snr=[valid_snr], data_per=0.2, phase='valid', dataset_name=['CDL-A'],RX_Ants_list=RX_Ants_list)
        print(vl_input.shape)
        ...
        testset_new = MyDataset(x = vl_input, snr=vl_snr, dop=vl_dop, label = vl_label)
        testloader_new = torch.utils.data.DataLoader(testset_new, batch_size=my_batch_size, shuffle=False, num_workers=8)
        print('New CDL-A testset len=',len(testset_new))
        ...
        print('Old CDL-A testset len=',len(testset))
        ...
        for idx, data in enumerate(testloader): #enumerate(trainloader, 0):
            inputs,ts_snr,ts_dop,labels =data # ,_
            print(' labels ',labels.shape)
            print(' ts_snr ',ts_snr.shape)
            print(' ts_dop ',ts_dop.shape)
        ...
        #print('Valid old CDL-A, BER={:.6f}, snr={}, dop={}'.format(valid_ber.item(), -round(10.0*torch.log10(ts_snr.mean()).cpu().numpy()) , ts_dop.mean()))
        print('Valid CDL-A, BER={:.6f}, snr={}, dop={}'.format(valid_ber.item(), -round(10.0*torch.log10(ts_snr.mean()).cpu().numpy()) , ts_dop.mean()))
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for epoch in range(start, self.epoch):
                ...
                for idx, data in enumerate(trainloader):
                    inputs, tr_snr_batch,_,labels =data # ,_
                    inputs, labels = inputs.to(device),labels.to(device)
                    labels = labels.float()#
                    labels = torch.clamp(labels, 0, 1)#
                    #print("outputs shape", outputs.shape, "labels shape", labels.shape)#
                ...
                test_ber_cnt = 0
                test_loss_cnt=0
                print('Phase: valid Old CDL-A *******************')
                self.net.eval()
                for idx, data in enumerate(testloader): #enumerate(trainloader, 0):
                    inputs,_,_,labels =data # ,_
                    inputs, labels = inputs.to(device),labels.to(device)
                    with torch.no_grad():
                        outputs=self.net(inputs)
                        labels=labels.permute(0,2,3,1)
                ...
                print('Epoch {:d}, old CDL-A, BER={:.6f}, epoch_loss = {:,.6f}'.format(epoch, old_eva_ber, old_eva_loss))
                
                test_ber_cnt = 0
                test_loss_cnt=0
                print('Phase: valid new CDL-A *******************')
                self.net.eval()
                ...
                print('Epoch {:d}, New CDL-A, BER={:.6f}, epoch_loss = {:,.6f}'.format(epoch, new_eva_ber, new_eva_loss))
   
                print('\tOld CDL-A (BER) \t New CDL-A (BER) \t Old CDL-A (Loss) \t New CDL-A (Loss)')
                ...
                plt.plot(epoch_list, ber_list,'b',label='Validation BER, old CDL-A')
                plt.plot(epoch_list, ber_new_list,'r',label='Validation BER, new CDL-A')
                plt.ylabel('BER')
                plt.xlabel('Epoch')
                plt.legend()
